...
impl Key {
        ...
        S: AsRef<str>,
    {
        ...
        Self::from_segments(&self.segments[prefix.len()..])
    }
    ...
    fn check_key<S1, S2>(name: S1, _segments: &[S2]) -> Result<()>
    where
        S1: AsRef<str>,
        S2: AsRef<str>,
    {
...
#[cfg(test)]
mod test {
    use super::{Key, MAX_KEY_NAME_LENGTH};
    ...
    fn dotted_data_key_ok() {
        assert!(Key::new("a.b.c.d.e.f.g").is_ok());
    }
    ...
    fn from_segments() {
        ...
        let key = Key::from_segments(segments).unwrap();
        assert_eq!(key.name(), name);
    ...
    #[test]
    fn long_key_ok() {
        assert!(Key::new("a".repeat(MAX_KEY_NAME_LENGTH)).is_ok());
    }

    #[test]
    fn key_too_long() {
        assert!(Key::new("a".repeat(MAX_KEY_NAME_LENGTH + 1)).is_err());
    }
    ...
    fn strip_prefix_ok() {
        // Remove plain prefix
        let key = Key::new("a.b.c.d").unwrap();
        let prefix = "a.b";
    ...
    #[test]
    fn append_key_err() {
        let long_key = Key::new("a".repeat(MAX_KEY_NAME_LENGTH)).unwrap();
